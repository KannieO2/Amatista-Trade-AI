...
def accumulation_score(W: list[dict]) -> tuple[int, dict]:
    ...
    f_imb = 1.0 if HEALTHY_IMB_LO <= imb <= HEALTHY_IMB_HI else 0.0  # sesgo comprador sano
...
def rug_risk_score(W: list[dict]) -> tuple[int, dict]:
    ...
    trap = 0
    for (a, pa), (b, pb) in zip(zip(price, bid), zip(price[1:], bid[1:])):
        dmove = abs(pb - a) / (abs(a) + EPS)
        if dmove > MOVE_NO_SUPPORT_PCT and pb < med_bid:
            trap += 1
