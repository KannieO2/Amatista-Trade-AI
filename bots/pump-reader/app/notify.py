...
# up to the first existing .env, else fall back to the app dir (env vars are the
# real source in the container; the file is only used to PERSIST Settings-UI edits).
...
def format_alert(symbol: str, status_label: str, exchange: str = "",
                 liquidity_usd: float = 0.0, reasons: list[str] | None = None,
                 odds: str = "", importance: str = "") -> str:
    """A pre-pump SIGNAL (heads-up, NOT an entry). Result-oriented, plain words: the
    status_label is the headline (ACUMULANDO/INMINENTE/ON-CHAIN), `reasons` say WHY in
    human terms, `odds` is the honest read. No internal jargon (acc/pers/rug/cluster)."""
    liq = f"${liquidity_usd:,.0f}" if liquidity_usd else "—"
    venue = exchange.title() if exchange else "—"
    why = " · ".join(reasons) if reasons else "acumulación sostenida"
    imp = f"  ·  <b>{importance}</b>" if importance else ""
    odds_line = f"\n🔮 <b>Probabilidad:</b> {odds}" if odds else ""
    return (
        f"<b>[PUMP]</b> {status_label}  ·  <b>{symbol}</b> ({venue}){imp}\n"
        f"\n"
        f"💧 <b>Liquidez:</b> {liq}\n"
        f"📈 <b>Por qué:</b> {why}"
        f"{odds_line}"
...
def format_entry(symbol: str, exchange: str, price: float, accel: float, score: int,
                 ...
                 timeout_min: float, trail_arm: float, trail_give: float,
                 confidence: float, setup: str = "", prob: float | None = None,
                 prob_n: int = 0) -> str:
    """New position opened — with the active protections spelled out. `setup` tags
    WHERE in the cycle the buy happened (pre-pump líder / pre-pump ruptura / gainers)
    so the notification reads as the INVESTMENT itself and shows if it was early.
    `prob` = empirical P(sube) for this setup's bucket (honest, from track record)."""
    flag_text = ", ".join(flags) if flags else "ninguna"
    head = f"INVERSIÓN · {setup}" if setup else "NUEVA OPERACIÓN"
    prob_line = f"🔮 <b>P(sube):</b> {prob:.0%} (n={prob_n})\n" if prob is not None else ""
    return (
        f"<b>[PUMP]</b> 🚀 <b>{head}</b>  ·  {symbol}\n"
        f"\n"
        ...
        f"🎚️ <b>Confianza:</b> {confidence:.0f}/100\n"
        f"\n"
        f"🛡️ <b>Protecciones activas</b>\n"
        f"💥 Panic sell  ·  dump {dump_pct:g}x\n"
        f"⏳ Time-out  ·  {timeout_min:g}m lateral\n"
        f"📈 Trailing  ·  desde +{trail_arm:g}%, asegura {100 - trail_give:g}% del pico"
    )
