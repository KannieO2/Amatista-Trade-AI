...
class Pipeline:
    ...
    def state_of(self, symbol: str, exchange: str) -> str | None:
        """Estado FSM actual de un símbolo (o None si no está en el embudo).
        Lo usa el acelerador de velocity: solo dispara la ruptura de un token que
        YA pasó acumulación/persistencia/rug y está en 'confirmation'."""
        symbol, exchange = self._key(symbol, exchange)
        row = self._get(symbol, exchange)
    ...
    def reconcile_entries(self, open_keys: set[tuple[str, str]]) -> int:
        """Expira las filas en estado 'entry' que NO tienen posición real abierta.
        Arregla el 'COMPRADO fantasma': fsm_state es SQLite persistente, así que un
        'entry' viejo (cuyo cierre no llamó mark_closed, o sobrevivió a un reinicio)
        seguía contando como comprado aunque no exista la posición. open_keys =
        {(SYMBOL_UPPER, exchange_lower)} de las posiciones realmente abiertas."""
        try:
            with self._lock:
                self._conn.row_factory = sqlite3.Row
                rows = self._conn.execute(
                    "SELECT symbol,exchange FROM fsm_state WHERE state='entry'").fetchall()
                self._conn.row_factory = None
                stale = [(r["symbol"], r["exchange"]) for r in rows
                         if (r["symbol"].upper(), r["exchange"].lower()) not in open_keys]
                now = int(time.time() * 1000)
                for sym, ex in stale:
                    self._conn.execute(
                        "UPDATE fsm_state SET state='expired', updated_at=? "
                        "WHERE symbol=? AND exchange=? AND state='entry'", (now, sym, ex))
                self._conn.commit()
            for sym, ex in stale:
                self._log(sym, ex, "entry", "expired", "reconcile_orphan")
            return len(stale)
        except Exception:
            logger.exception("reconcile_entries failed")
            return 0
