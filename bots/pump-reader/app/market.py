...
async def market_for_symbol(base_symbol: str) -> dict | None:
    ...
    # Fallback CMC: CoinGecko pierde muchos microcaps; CMC los cubre. Mismo shape.
    if data is None:
        try:
            from . import coinmarketcap as _cmc
            data = await _cmc.quotes(key)
        except Exception:
            logger.debug("cmc fallback failed: %s", key, exc_info=True)
