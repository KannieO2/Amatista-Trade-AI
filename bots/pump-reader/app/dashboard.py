...
<style>
  ...
  *::-webkit-scrollbar{width:0;height:0;display:none}
  ...
  /* Sticky header for scrollable tables: sticky must sit on the TH cells (not the
     THEAD) or Chrome leaves a translucent gap that scrolled rows bleed through. */
  .stickyhd thead th{position:sticky;top:0;background:var(--inset);z-index:2;padding-top:9px;box-shadow:inset 0 -1px 0 var(--border)}
  tbody td{padding:8px;border-top:1px solid var(--border-soft);font-size:12px;vertical-align:middle}
  /* row hover — scannability + signals the row is interactive (opens detail) */
  tbody tr{transition:background .12s ease}
  tbody tr:hover{background:linear-gradient(90deg,rgba(160,92,242,.07),rgba(160,92,242,.015))}
  /* edge-health / KPI stat strip — boxed metrics with a tinted divider so the
     numbers read as a dashboard, not a flat run-on row */
  .estrip{display:flex;flex-wrap:wrap;gap:0;margin-bottom:14px;border:1px solid var(--border-soft);border-radius:11px;overflow:hidden;background:var(--panel-2)}
  .estat{flex:1;min-width:108px;padding:11px 14px;border-left:1px solid var(--border-soft)}
  .estat:first-child{border-left:none}
  .estat .el{font-size:9px;letter-spacing:.1em;color:var(--muted);text-transform:uppercase}
  .estat .ev{font-family:"Geist Mono",monospace;font-weight:600;margin-top:6px;letter-spacing:-.01em;line-height:1}
  ...
  /* Prose reads as authored (sentence case). The old global text-transform:capitalize
     Title-Cased EVERY word and mangled Spanish ("Monta El Movimiento", "Salud Del
     Edge"). Structural labels keep their own explicit uppercase below. */
  /* Gainers view: compact P&L stat grid + velocity watch-list. */
  .gstats{display:grid;grid-template-columns:repeat(auto-fit,minmax(92px,1fr));gap:8px}
  .gwatch-row{display:flex;justify-content:space-between;align-items:center;padding:7px 2px;border-bottom:1px solid var(--border-soft);font-size:12px}
  .gwatch-row:last-child{border-bottom:0}
  .gwatch-accel{font-weight:700;font-variant-numeric:tabular-nums}
  /* book badge on live position cards: which engine bought it. */
  .bookbadge{font-size:8.5px;font-weight:700;letter-spacing:.06em;padding:2px 6px;border-radius:999px;vertical-align:middle}
  .bb-p{background:rgba(80,160,255,.14);color:#5aa0ff}
  .bb-g{background:rgba(232,170,60,.16);color:#e8aa3c}
  .bb-r{background:rgba(240,136,62,.16);color:#f0883e}
  .closebtn{background:rgba(240,70,90,.14);color:#ff5a6e;border:1px solid rgba(240,70,90,.3);border-radius:6px;cursor:pointer;font-size:11px;line-height:1;padding:3px 6px;transition:background .12s}
  .closebtn:hover{background:rgba(240,70,90,.28)}
  .spin{display:inline-block;width:11px;height:11px;border:2px solid var(--border-soft,#2a2f3a);border-top-color:var(--accent,#a855f7);border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle}
  @keyframes spin{to{transform:rotate(360deg)}}
  #btn-discover.loading{opacity:.75;pointer-events:none}
  #btn-discover{display:inline-flex;align-items:center;gap:6px;min-width:118px;justify-content:center}
  #btn-discover .dc{font-size:10px;color:var(--muted-2,#aeb4c2);opacity:.8;font-variant-numeric:tabular-nums;margin-left:2px}
  #btn-discover.loading{opacity:.85;pointer-events:none}
  /* --- Entradas rechazadas: gráfico de barras + tonos --- */
  .rj-sub{font-size:11.5px;color:var(--muted,#8b93a7);line-height:1.6;margin:-2px 0 14px;max-width:62ch}
  .rj-chart{display:flex;flex-direction:column;gap:8px;margin-bottom:18px}
  .rjbar{display:grid;grid-template-columns:170px 1fr 34px;align-items:center;gap:11px}
  .rjbar .rjl{font-size:11.5px;color:var(--text-2,#c7ccd6);display:flex;align-items:center;gap:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .rjdot{width:7px;height:7px;border-radius:50%;flex:0 0 auto}
  .rjtrack{height:9px;border-radius:999px;background:rgba(255,255,255,.045);overflow:hidden}
  .rjfill{height:100%;width:100%;border-radius:999px;transform-origin:left;transform:scaleX(0);transition:transform .7s cubic-bezier(.16,1,.3,1)}
  .rjn{font-size:12px;text-align:right;color:var(--muted-2,#aeb4c2)}
  .rj-cap{font-size:10.5px;letter-spacing:.08em;text-transform:uppercase;color:var(--muted,#8b93a7);margin:2px 0 9px}
  .rjtag{display:inline-flex;align-items:center;gap:7px;font-size:12px}
  .tone-danger{background:#ff5a6e}.tone-warn{background:#e8aa3c}.tone-info{background:#5aa0ff}.tone-good{background:#34c759}.tone-muted{background:#8b93a7}
  .rjfill.tone-danger{background:linear-gradient(90deg,rgba(255,90,110,.45),#ff5a6e)}
  .rjfill.tone-warn{background:linear-gradient(90deg,rgba(232,170,60,.45),#e8aa3c)}
  .rjfill.tone-info{background:linear-gradient(90deg,rgba(90,160,255,.45),#5aa0ff)}
  .rjfill.tone-good{background:linear-gradient(90deg,rgba(52,199,89,.45),#34c759)}
  .rjfill.tone-muted{background:linear-gradient(90deg,rgba(139,147,167,.35),#8b93a7)}
  @keyframes rjrow{from{opacity:0;transform:translateY(5px)}to{opacity:1;transform:none}}
  .rj-tbl tbody tr{animation:rjrow .35s cubic-bezier(.16,1,.3,1) both}
  .cfg-card{border:1px solid var(--border-soft);border-radius:11px;padding:13px 15px;margin-top:12px}
  .cfg-card:first-of-type{margin-top:4px}
  .cfg-cap{font-weight:700;font-size:12.5px;letter-spacing:.05em;margin-bottom:11px}
  .cfg-gain{border-color:rgba(52,199,89,.28);background:rgba(52,199,89,.045)}
  .cfg-pump{border-color:rgba(90,160,255,.28);background:rgba(90,160,255,.05)}
  /* Pre-pump vs Gainers P&L split (two strategy buckets). */
  .bsplit{display:flex;gap:10px;margin-top:14px}
  .bcard{flex:1;background:var(--inset);border:1px solid var(--border);border-radius:10px;padding:11px 13px}
  .bname{font-size:11px;font-weight:600;display:flex;justify-content:space-between;align-items:baseline}
  .bsub{font-size:9px;letter-spacing:.08em;color:var(--muted);text-transform:uppercase}
  .bpnl{font-size:20px;font-weight:700;margin:4px 0 2px}
...
<div class="app">
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <!-- "Tokens" (monitoreo/análisis genérico de mercado) retirado del nav a
           propósito: eso lo hace toda la competencia. El edge de Amatista es la
           detección pre-pump (Alerts) + el análisis real (Fase 2 FSM). La vista y
           su endpoint siguen existiendo por si se quiere reactivar. -->
  ...
  <div class="main">
    ...
    <section class="view" id="view-pump">
      ...
      <div class="grid-2">
        ...
        <div class="panel">
          ...
          <div class="bsplit" id="book-split"></div>
        </div>
      </div>
    ...
    <section class="view hidden" id="view-alerts">
      <div class="vhead"><div><h1>Alerts</h1><p>Detección temprana · cuánto subió cada token DESDE que el bot lo alertó</p></div><div class="ts mono" id="al-ts">—</div></div>
      <div class="panel">
        <div class="phead"><span class="pt">Prueba de detección temprana</span><span class="px" id="al-proof-count">0</span></div>
        <div id="al-proof"><div class="empty">Sin alertas registradas todavía</div></div>
      </div>
      <div class="panel" style="margin-top:14px">
        <div class="phead"><span class="pt">Cola de confirmación (en vivo)</span><span class="px" id="al-count">0</span></div>
    ...
    <section class="view hidden" id="view-learning">
      ...
      <div class="grid-kpi">
        ...
        <div class="card"><div class="klabel">Auto-ajustes activos</div><div class="kval" id="lrn-prop-n">0</div><div class="ksub" id="lrn-prop-sub">el bot afina solo</div></div>
      </div>
      <div class="panel">
        <div class="phead"><span class="pt">Auto-ajustes del learning</span><span class="px" id="lrn-prop-c">0</span></div>
        <div id="lrn-proposals"><div class="empty">El learning necesita outcomes asentados (horizonte 7d) antes de auto-ajustar. Arranca ~7 días tras el deploy.</div></div>
      </div>
      <div class="panel">
      ...
      <div class="panel">
        <div class="phead"><span class="pt">Entradas Rechazadas · El Bot No Compró</span><span class="px mono" id="rj-total">—</span></div>
        <div class="rj-sub">Prueba Viva De Los Filtros — Cada Barra Es Un Motivo Por El Que El Bot Se Abstuvo De Comprar.</div>
        <div id="rj-chart" class="rj-chart"><div class="empty" style="padding:14px 0">Cargando…</div></div>
        <div class="rj-cap">Últimos Rechazos</div>
        <table class="rj-tbl"><thead><tr><th>Hora</th><th>Token</th><th>Motivo</th><th>Detalle</th></tr></thead>
      ...
      <div class="panel">
        <div class="phead"><span class="pt">Aprendizaje diferenciado · nada se borra</span><span class="px" id="lb-sub">3 categorías</span></div>
        <div class="grid-kpi" style="margin-bottom:10px">
          <div class="card"><div class="klabel">Successful_Trades</div><div class="kval" id="lb-success" style="color:var(--green)">0</div><div class="ksub">trade ejecutado</div></div>
          <div class="card"><div class="klabel">Failed_Attempts</div><div class="kval" id="lb-failed">0</div><div class="ksub">no pudo entrar (benigno)</div></div>
          <div class="card"><div class="klabel">Dangerous_Signals</div><div class="kval" id="lb-danger" style="color:var(--red)">0</div><div class="ksub">estafa/rug · se evita activamente</div></div>
        </div>
        <div class="px" id="lb-active" style="color:var(--muted)">—</div>
      </div>
    ...
    <section class="view hidden" id="view-trades">
      ...
      <div class="panel" id="wr-panel" style="margin-bottom:14px">
        ...
        <div class="estrip">
          <div class="estat"><div class="el">Win rate real</div><div class="ev" id="wr-win" style="font-size:26px">—</div></div>
          <div class="estat"><div class="el">Break-even</div><div class="ev" id="wr-be" style="font-size:19px;color:var(--muted-2)">—</div></div>
          <div class="estat"><div class="el">Margen</div><div class="ev" id="wr-margin" style="font-size:19px">—</div></div>
          <div class="estat"><div class="el">Profit factor</div><div class="ev" id="wr-pf" style="font-size:19px">—</div></div>
          <div class="estat"><div class="el">Expectancy/trade</div><div class="ev" id="wr-exp" style="font-size:19px">—</div></div>
          <div class="estat"><div class="el">Trades</div><div class="ev" id="wr-n" style="font-size:19px">—</div></div>
        </div>
      ...
      <div class="grid-2b">
        <div class="panel">
          ...
          <table><thead><tr><th>Token</th><th>Entry</th><th>Last</th><th>Gain</th><th>Conf</th><th>Unreal.</th></tr></thead>
        ...
        <div class="panel">
          ...
          <table class="stickyhd"><thead><tr><th>Token</th><th>Reason</th><th>%</th><th>Price</th><th>PnL</th></tr></thead>
    ...
    <section class="view hidden" id="view-pipeline">
      ...
      <div class="panel" style="margin-bottom:14px">
        ...
        <table class="stickyhd"><thead><tr><th>Token</th><th>Exch</th><th>Estado</th><th title="Acumulación: presión compradora sostenida (0-100)">Acum</th><th title="Persistencia: cuánto SOSTIENE la señal (0-100)">Persist</th><th title="Riesgo de rug/estafa (más bajo = mejor)">Rug</th><th title="Bonus de secuencia: empuje encadenado en velas">Secuencia</th><th title="Ticks de confirmación sostenidos antes de comprar">Confirm</th></tr></thead>
    ...
    <section class="view hidden" id="view-settings">
      ...
      <div class="panel" style="margin-top:16px">
        <div class="phead"><span class="pt">Bot configuration</span><span class="px">Aplica en vivo · dos motores independientes</span></div>

        <!-- ===== GENERAL (compartido por ambos motores) ===== -->
        <div class="cfg-card">
          <div class="cfg-cap">⚙️ GENERAL · compartido</div>
          <div class="gform">
            <div class="gf"><label>Umbral de confirmación</label><input id="cfg-thr" type="number" min="1" max="100" step="1" class="mono" /></div>
            <div class="gf"><label>Auto-entry (paper)</label><select id="cfg-auto"><option value="true">Activado</option><option value="false">Desactivado</option></select></div>
            <div class="gf"><label>Tamaño de entrada (USDT)</label><input id="cfg-size" type="number" min="10" step="1" class="mono" /></div>
            <div class="gf"><label>Máx operaciones simultáneas <span class="px">0 = sin límite</span></label><input id="cfg-maxopen" type="number" min="0" step="1" class="mono" /></div>
          </div>
        </div>
...
function wireEquityHover(curve){
  ...
  const innerW=(EQ_W-pad*2)||1;
  wrap.onmousemove=(ev)=>{
    const r=wrap.getBoundingClientRect();
    let fx=(ev.clientX-r.left)/r.width; fx=Math.max(0,Math.min(1,fx));
    // INVERTE el mismo mapeo X del SVG (viewBox EQ_W de ancho, padding a cada lado,
    // preserveAspectRatio=none). Sin esto el punto/línea iban corridos vs la curva
    // real (la inconsistencia del hover): el SVG plotea con padding+escala 584/600
    // pero el hover usaba r.width directo. Ahora coinciden exacto.
    const vbx=fx*EQ_W;                                   // cursor en coords del viewBox
    let i=Math.round((vbx-pad)/innerW*(n-1));
    i=Math.max(0,Math.min(n-1,i));
    const v=nums[i];
    // px del dot/línea = posición REAL del punto i en el SVG, reescalada a píxeles.
    const px=(((i/(n-1))*innerW+pad)/EQ_W)*r.width;
    const py=h-pad-((v-min)/rng)*(h-pad*2);
...
// ---- render overview ----
function renderBookSplit(books){
  const el=$("book-split"); if(!el) return;
  if(!books){ el.innerHTML=""; return; }
  const card=(name,sub,b)=>{
    const pnl=(b&&b.pnl!=null)?b.pnl:0;
    const col=pnl>0?'var(--green)':pnl<0?'var(--red)':'var(--muted-2)';
    const sign=pnl>=0?'+':'-';
    return `<div class="bcard">
      <div class="bname">${name}<span class="bsub">${sub}</span></div>
      <div class="bpnl mono" style="color:${col}">${sign}$${Math.abs(pnl).toFixed(2)}</div>
...
async function loadOverview(){
  ...
  renderBookSplit(d.books);

  // Vista unificada (criminal pumps): análisis FSM (estado/acc/pers/rug) + mercado
  // (score/cluster/Δ24h/chart) en una sola tabla rica. El backend enriquece cada fila.
  const pp=(d.prepump||[]);
...
// ---- Discover (barrido full, 1×/día): spinner + countdown DENTRO del botón ----
// Discover = busca tokens NUEVOS en TODO el exchange y depura el universo (1×/día).
// Update (botón aparte) = chequea oportunidad de entrar ahora (cada 3 min).
let _updInterval=180, _lastUpdMs=0, _discInterval=86400, _lastDiscMs=0, _manualScan=false, _lastStatusAt=0;
const DISCOVER_ICON='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4-4"/></svg>';
async function refreshScanStatus(){
  _lastStatusAt=Date.now();
  try{ const s=await (await fetch("/status")).json();
    _updInterval=s.scan_interval_seconds||_updInterval;
    _discInterval=s.discover_interval_seconds||_discInterval;
    if(s.last_scan_at) _lastUpdMs=new Date(s.last_scan_at).getTime();
    if(s.last_discover_at) _lastDiscMs=new Date(s.last_discover_at).getTime();
  }catch(e){}
}
function fmtRem(sec){
...
function renderDiscover(){
  const b=$("btn-discover"); if(!b) return;
  let scanning=_manualScan, cd='';
  if(!_manualScan && _lastDiscMs){
    const rem=_discInterval-(Date.now()-_lastDiscMs)/1000;
    if(rem<=1){ scanning=true; if(Date.now()-_lastStatusAt>3000) refreshScanStatus(); }
    else cd='<span class="dc">'+fmtRem(rem)+'</span>';
  }
  if(scanning){ b.classList.add("loading"); b.innerHTML='<span class="spin"></span>Descubriendo…'; }
  else{ b.classList.remove("loading"); b.innerHTML=DISCOVER_ICON+'Discover'+cd; }
}
$("btn-discover").addEventListener("click", async ()=>{
  if(_manualScan) return; _manualScan=true; renderDiscover();
  try{ await fetch("/scan?full=true",{method:"POST"}); await loadOverview(); await refreshScanStatus(); }catch(e){}
  _manualScan=false; renderDiscover();
});
refreshScanStatus(); setInterval(renderDiscover,1000); setInterval(refreshScanStatus,5000);
...
let TOKENS=[];
// Honest entry-timing badge. The book bucket alone LIED ("PRE-PUMP" on a position
// that entered ON the breakout). Now: lead = on-chain, bought BEFORE the venue move
// (truly pre-pump) · ruptura = FSM waited for the confirmed start, entered ON the
// candle (NOT pre-pump, honest) · momentum/gainers = chased a move already running.
function entryBadge(o){
  if(o.book==='gainers') return `<span class="bookbadge bb-g">MOMENTUM</span>`;
  const k=o.entry_phase||'ruptura';
  if(k==='lead') return `<span class="bookbadge bb-p">PRE-PUMP</span>`;
  if(k==='momentum') return `<span class="bookbadge bb-g">MOMENTUM</span>`;
  return `<span class="bookbadge bb-r">RUPTURA</span>`;
}
...
async function loadAlerts(){
  $("al-ts").textContent=new Date().toLocaleTimeString();
  // live confirmation queue
  try{
    const c=await (await fetch("/candidates")).json();
    const al=c.filter(t=>t.status==="waiting_confirmation");
    $("al-count").textContent=al.length+" active";
  ...
  // early-detection proof feed
  try{
    const p=await (await fetch("/alerts/performance")).json();
    const rows=p.alerts||[];
    $("al-proof-count").textContent=rows.length+" alertas";
    $("al-proof").innerHTML = rows.length ? rows.map(a=>{
      const mv=a.move_pct, up=mv>=0, hot=mv>=20;
      const nowc=a.now_pct>=0;
      return `<div class="alert" style="cursor:pointer" onclick="openCandidate('${a.symbol}','${a.exchange}')">${scoreBadge(a.pump_score)}
        <div class="meta"><div class="top"><b>${a.symbol}</b><span class="px">${upx(a.exchange)}</span>
          <span class="tag"><span class="cdot" style="background:${clusterColor(a.cluster)}"></span>${tcase(a.cluster)}</span>${labelPill(a.label)}</div>
          <div class="sub">Alertado @ <span class="mono">${a.alert_price}</span> · ahora <span class="mono">${a.last_price}</span> · pico desde alerta</div></div>
        <div style="text-align:right">
          <div class="mono" style="font-size:17px;font-weight:700;color:${hot?'#7CFFB2':(up?'var(--green)':'var(--red)')}">${up?'+':''}${mv}%</div>
          <div class="px mono" style="color:${nowc?'var(--green)':'var(--red)'}">ahora ${nowc?'+':''}${a.now_pct}%</div></div></div>`;
    }).join("")
      : `<div class="empty">Aún no hay alertas con histórico (aparecen cuando el bot alerta y registra precio)</div>`;
  }catch(e){ $("al-proof").innerHTML='<div class="empty">no se pudo cargar el histórico de alertas</div>'; }
}
...
const COMP_NAMES={volume_spike:"Volumen",price_change_pct_24h:"Cambio 24h",orderbook_imbalance:"Imbalance libro",liquidity_usd:"Liquidez"};
function compHtml(c){
  if(!c) return '<div class="empty">—</div>';
  if(!c.ready) return `<div class="empty">Aún sin muestras suficientes (hay ${c.have||0}, faltan ${c.need}).</div>`;
  const items=c.contrib||[];
  if(!items.length) return '<div class="empty">—</div>';
  // lift normalizado [-1,1]: separación relativa confirmed-vs-dud. + = ese valor
  // ALTO acompaña al pump; - = valor BAJO lo acompaña (ej liquidez baja = microcap).
  return items.map(x=>{ const up=x.lift>=0; const pct=Math.round(Math.min(Math.abs(x.lift),1)*100);
    const name=COMP_NAMES[x.signal]||tcase(x.signal);
    return `<div style="display:flex;align-items:center;gap:10px;margin:8px 0">
    <span style="width:120px;font-size:12px;color:var(--muted-2)">${name}</span>
    <div style="flex:1;height:7px;border-radius:4px;background:var(--inset);overflow:hidden"><i style="display:block;height:100%;width:${pct}%;background:${up?'var(--green)':'var(--red)'}"></i></div>
    <span class="mono" style="width:52px;text-align:right;font-size:12px;color:${up?'var(--green)':'var(--red)'}">${up?'+':''}${(x.lift*100).toFixed(0)}%</span></div>`; }).join("");
}
...
async function loadAutotune(){
  ...
  $("at-body").innerHTML=(a.rows||[]).map(r=>{
    ...
    return `<tr><td>${cap1(r.param)}</td><td class="mono">${r.value}</td><td>${tag}</td>
      <td class="px" style="color:var(--muted)">${cap1(r.how)}</td></tr>`;
  }).join("");
  $("at-note").textContent=cap1(a.note||"");
}
async function loadRejections(){
  let a; try{ a=await (await fetch("/entry-rejections")).json(); }catch(e){ return; }
  const total=a.total||0;
  $("rj-total").textContent=total+(total===1?" Rechazo":" Rechazos");
  const reasons=a.by_reason||[];
  const max=Math.max(1,...reasons.map(r=>r.count));
  const chart=$("rj-chart");
  if(!reasons.length){
    chart.innerHTML='<div class="empty" style="padding:14px 0">Sin Rechazos Aún</div>';
  }else{
    chart.innerHTML=reasons.map(r=>
      `<div class="rjbar">
        <span class="rjl"><span class="rjdot tone-${r.tone}"></span>${r.label}</span>
        <span class="rjtrack"><span class="rjfill tone-${r.tone}" data-w="${r.count/max}"></span></span>
        <span class="rjn mono">${r.count}</span></div>`).join("");
    // Anima el relleno (scaleX 0→proporción) con cascada escalonada — solo transform.
    requestAnimationFrame(()=>{ document.querySelectorAll('#rj-chart .rjfill').forEach((el,i)=>{
      el.style.transitionDelay=(i*55)+'ms'; el.style.transform='scaleX('+el.dataset.w+')'; }); });
  }
  const rows=a.recent||[];
  $("rj-body").innerHTML=rows.length ? rows.map((r,i)=>
    `<tr style="animation-delay:${Math.min(i*28,420)}ms"><td class="mono px">${new Date(r.t).toLocaleTimeString()}</td>
     <td class="sym">${r.symbol}</td>
     <td><span class="rjtag"><span class="rjdot tone-${r.tone}"></span>${r.label}</span></td>
...
async function loadBuckets(){
  let a; try{ a=await (await fetch("/learning/buckets")).json(); }catch(e){ return; }
  const c=a.counts||{};
  $("lb-success").textContent=c.successful||0;
  $("lb-failed").textContent=c.failed||0;
  $("lb-danger").textContent=c.dangerous||0;
  const act=a.dangerous_active||[];
  $("lb-active").textContent=act.length ? ("Evitando activamente: "+act.join(", ")) : "Sin tokens peligrosos activos";
}
async function loadLearning(){
  ...
  $("lrn-prop-sub").textContent = props.length?"el bot lo aplica solo (cada 1h)":"sin propuestas activas";
  $("lrn-proposals").innerHTML = props.length
    ? props.map(p=>`<div class="alert"><span class="badge" style="background:#15301f;color:#7CFFB2">AUTO</span><div class="meta"><div class="sub" style="color:var(--text)">${p.text} <span style="color:var(--muted)">· el learning lo ajusta solo, no tienes que hacer nada</span></div></div></div>`).join("")
    : `<div class="empty">El learning necesita outcomes asentados (horizonte 7d) antes de auto-ajustar. Arranca ~7 días tras el deploy.</div>`;
...
async function loadTrades(){
  ...
  // Pre-pump only here — gainers (velocity/momentum) live in their own Gainers tab.
  const mOpen=(m.open||[]).filter(o=>o.book!=='gainers');
  const mExits=(m.exits||[]).filter(e=>e.book!=='gainers');
  // managed open positions
  $("mg-count").textContent=mOpen.length+" open";
  $("mg-thr").textContent="thr "+m.adaptive_threshold;
  $("mg-body").innerHTML=mOpen.length ? mOpen.map(o=>{
    ...
    return `<tr><td class="sym">${o.symbol} <span class="px">${upx(o.exchange)}</span></td><td class="mono px">${o.entry_price}</td>
      ...
      <td class="mono px">${o.confidence??100}</td>
  ...
  const _open=mOpen;   // pre-pump only (gainers shown in Gainers tab)
    ...
    $("mg-charts").innerHTML=_open.length ? _open.map(o=>{
      ...
      return `<div style="border:1px solid var(--border-soft);border-radius:10px;overflow:hidden">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:7px 10px">
          <span class="sym">${o.symbol} <span class="px">${upx(o.exchange)}</span> ${badge}</span>
          <span style="display:flex;align-items:center;gap:8px">
          <span class="mono px" style="color:${up?'var(--green)':'var(--red)'}" data-mgpnl="${o.exchange}:${o.symbol}">${up?'+':''}${o.gain_pct}% · ${o.unrealized_pnl>=0?'+':''}${o.unrealized_pnl}</span>
          <button class="closebtn" title="Cerrar posición" onclick="closePos('${o.exchange}','${o.symbol}')">✕</button></span></div>
  ...
  $("mg-exits").innerHTML=mExits.length ? mExits.map(e=>
...
async function loadSettings(){
  ...
  try{
    ...
    $("cfg-maxopen").value=cfg.max_open_trades;
...
async function saveConfig(){
  const body={
    ...
    max_open_trades:Math.max(0,Math.floor(Number($("cfg-maxopen").value)||0)),
    gainers_accel:Number($("cfg-accel").value),
    pump_breakout_pct:Number($("cfg-pump-breakout").value),
    pump_vol_spike:Number($("cfg-pump-vol").value),
  };
...
function onchainBlock(o){
  if(!o) return `<div class="navlabel" style="padding:0 0 6px;color:var(--muted)">⛓ On-chain holders · no disponible (token sin contrato resoluble o cadena no soportada)</div>`;
  const conc=o.concentrated;
  const chip=(label,val,bad)=>`<div class="sbox"><div class="l">${label}</div><div class="v mono" style="color:${bad?'var(--red)':'var(--green)'}">${val}</div></div>`;
  const rows=(o.top||[]).map((h,i)=>`<div style="display:flex;align-items:center;gap:8px;margin:2px 0">
     <span class="px" style="width:74px">#${i+1}${h.tag?(' · '+h.tag):''}${h.is_contract?' ⚙':''}</span>
     <div style="flex:1;height:10px;background:var(--inset);border-radius:3px;overflow:hidden"><i style="display:block;height:100%;width:${Math.min(h.pct,100).toFixed(0)}%;background:${h.pct>=20?'rgba(255,90,120,.6)':'rgba(124,108,255,.5)'}"></i></div>
     <span class="mono px" style="width:54px;text-align:right">${h.pct}%</span></div>`).join("");
  return `<div class="navlabel" style="padding:0 0 6px">⛓ <b style="color:var(--green)">On-chain REAL</b> · GoPlus · chain ${o.chain}${conc?' · <b style="color:var(--red)">CONCENTRADO ⚠</b>':' · distribución sana'}</div>
    <div class="statgrid" style="margin-bottom:10px">
      ${chip('Holders', (o.holder_count||0).toLocaleString())}
      ${chip('Top-1 whale', o.top1_whale_pct+'%', o.top1_whale_pct>=25)}
      ${chip('Top-10 juntos', o.top10_pct+'%', o.top10_pct>=70)}
      ${chip('Creador', (o.creator_pct||0)+'%', (o.creator_pct||0)>=5)}
    </div>
    ${rows}`;
}
function holdersTab(d){
  const dp=d.depth||{};
  return `${onchainBlock(d.onchain)}
    <div class="navlabel" style="padding:14px 0 8px;color:var(--muted)">Orderbook depth · proxy CEX (respaldo)</div>
    ${depthLadder(dp)}
    <div class="statgrid" style="margin-top:12px">
...
function inflowsRealBlock(f){
  if(!f) return `<div class="navlabel" style="padding:0 0 6px;color:var(--muted)">⛓ Inflows on-chain reales · no disponibles (sin ETHERSCAN_API_KEY, o token no-EVM). Mostrando proxy CEX abajo.</div>`;
  const hot=(f.large_deposits||0)>0||(f.inflow_usd||0)>=100000;
  return `<div class="navlabel" style="padding:0 0 6px">⛓ <b style="color:var(--green)">Inflows REALES</b> · Etherscan · ${f.window} · chain ${f.chain}${hot?' · <b style="color:#f5b945">DEPÓSITOS GRANDES ⚠</b>':''}</div>
    <div class="statgrid" style="margin-bottom:10px">
      <div class="sbox"><div class="l">Inflow a exchanges</div><div class="v mono" style="color:${hot?'#f5b945':'var(--green)'}">${moneyC(f.inflow_usd||0)}</div></div>
      <div class="sbox"><div class="l">Depósitos</div><div class="v mono">${f.deposits||0}</div></div>
      <div class="sbox"><div class="l">Grandes (≥100k)</div><div class="v mono" style="color:${(f.large_deposits||0)>0?'#f5b945':'var(--muted)'}">${f.large_deposits||0}</div></div>
    </div>`;
}
function dexBlock(x){
  if(!x) return `<div class="navlabel" style="padding:0 0 6px;color:var(--muted)">🦄 DEX (DexScreener) · sin par DEX para este token</div>`;
  const r=x.buy_ratio_h24, bull=r!=null&&r>=0.55, bear=r!=null&&r<=0.45;
  const pill=(b,s)=>{const t=b+s||1;return `<div style="display:flex;height:12px;border-radius:3px;overflow:hidden;background:var(--inset)"><i style="width:${(b/t*100).toFixed(0)}%;background:rgba(70,220,160,.6)"></i><i style="width:${(s/t*100).toFixed(0)}%;background:rgba(255,90,120,.6)"></i></div>`;};
  return `<div class="navlabel" style="padding:0 0 6px">🦄 <b style="color:var(--green)">DEX REAL</b> · DexScreener (gratis) · ${x.dex} · ${x.chain}${bull?' · <b style="color:#7CFFB2">COMPRA domina</b>':(bear?' · <b style="color:var(--red)">VENTA domina</b>':'')}</div>
    <div class="statgrid" style="margin-bottom:8px">
      <div class="sbox"><div class="l">Liquidez DEX</div><div class="v mono">${moneyC(x.liquidity_usd)}</div></div>
      <div class="sbox"><div class="l">Vol 24h DEX</div><div class="v mono">${moneyC(x.vol_h24)}</div></div>
      <div class="sbox"><div class="l">Buys/Sells 24h</div><div class="v mono"><span style="color:var(--green)">${x.buys_h24}</span>/<span style="color:var(--red)">${x.sells_h24}</span></div></div>
      <div class="sbox"><div class="l">% compra 24h</div><div class="v mono" style="color:${bull?'var(--green)':(bear?'var(--red)':'var(--muted-2)')}">${r!=null?(r*100).toFixed(0)+'%':'—'}</div></div>
    </div>
    ${pill(x.buys_h24,x.sells_h24)}`;
}
function inflowsTab(d){
  ...
  return `${inflowsRealBlock(d.inflows)}
    <div style="padding:10px 0 0">${dexBlock(d.dex)}</div>
    <div class="navlabel" style="padding:12px 0 8px;color:var(--muted)">Volume inflow per ${d.timeframe} · proxy CEX (respaldo)</div>
...
async function openPnl(){
  ...
  const section=(label,color,arr)=> arr.length
    ...
    : "";
  // Un bloque por MOTOR (gainers vs scampump/pre-pump), cada uno con su total propio
  // + su desglose Ganando/Perdiendo. Reusa las tarjetas .cfg-card del panel de config.
  const bookBlock=(title,accent,arr)=>{
    if(!arr.length) return "";
    const tot=arr.reduce((s,r)=>s+r.total,0);
    const win=arr.filter(r=>r.total>0), lose=arr.filter(r=>r.total<0), flat=arr.filter(r=>r.total===0);
    return `<div class="cfg-card" style="border-color:${accent}44">
      <div class="cfg-cap" style="color:${accent};display:flex;justify-content:space-between;align-items:center">
        <span>${title}</span>
        <span class="mono" style="font-weight:700;color:${tot>=0?'var(--green)':'var(--red)'}">${sign(tot)} · ${win.length}↑ ${lose.length}↓</span>
      </div>`
      + section("▲ Ganando","var(--green)",win)+section("▼ Perdiendo","var(--red)",lose)+section("• Neutro","var(--muted)",flat)
      + `</div>`;
  };
  const gainers=d.rows.filter(r=>r.book==='gainers');
  const pump=d.rows.filter(r=>r.book!=='gainers');
  $("pnl-rows").innerHTML = d.rows.length
    ? bookBlock("⚡ GAINERS · momentum","#34c759",gainers)+bookBlock("🎯 SCAMPUMP · pre-pump","#5aa0ff",pump)
