...
class RiskGuard:
    ...
    def _local_eval(self, ctx: RiskContext) -> RiskDecision:
        ...
        # max_open_trades <= 0 means UNLIMITED (no concurrency cap). Other risk
        # gates (daily loss, drawdown, position size) still govern exposure.
        if limits.max_open_trades > 0 and ctx.open_trades >= limits.max_open_trades:
